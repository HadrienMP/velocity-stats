...
import fr.hadrienmp.stats.tickets.source.jira.Response
...
fun main(args: Array<String>) {
    ...
    val responseRaw = jiraTickets(credentials, jiraHost, project, LocalDate.now().minusMonths(6).withDayOfMonth(1))
    val response = Parser.parse<Response>(responseRaw.body())

    println(response)

    File("src/test/kotlin/fr/hadrienmp/stats/tickets/source/jira/client/tickets.json")
            .writeText(responseRaw.body(), defaultCharset())
}
